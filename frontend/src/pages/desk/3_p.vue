...
const { gameState, newRound, roundFinished } = useJudgementGame(3);
...
<template>
  <div class="flex h-screen w-screen items-end bg-gray-800 pb-5 ">
    ...
    <div class="relative flex h-1/3 w-full items-center justify-center">
      <Card
        v-for="(card, index) in gameState.players[0].cards" id="drag1" :key="card.img"
        :card="card" class="absolute origin-bottom" :class="[calculateRotationClass(0, index), calculateTranslationClass(0, index)]" @dragstart="drag"
      />
    </div>
